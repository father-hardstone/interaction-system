...
const IncompleteInteractionsTable = ({
    ...
    const [resumingInteractionId, setResumingInteractionId] = useState(null);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6 flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between mb-4 shrink-0">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">The interactions where any key fields are missing.</p>
                </div>
            </div>
            <div className="overflow-x-auto flex-1 min-h-0 border border-slate-100 rounded-lg">
                <table className="w-full border-collapse min-w-[700px]">
                    ...
                    <tbody>
                        {isLoading ? (
                            <tr>
                                <td colSpan={showOfficer ? 5 : 4} className="px-4 sm:px-6 py-16 text-center">
                            ...
                            <tr>
                                <td colSpan={showOfficer ? 5 : 4} className="px-4 sm:px-6 py-12 text-center text-slate-400 text-sm">
                                    ...
                                    <td className="px-4 sm:px-6 py-4 align-middle text-right" onClick={(e) => e.stopPropagation()}>
                                        {handleStartInteraction && (
                                            <button
                                                type="button"
                                                onClick={async () => {
                                                    if (blockResume || resumingInteractionId === interaction.id) return;
                                                    setResumingInteractionId(interaction.id);
                                                    try {
                                                        await handleStartInteraction(interaction.id);
                                                    } finally {
                                                        setResumingInteractionId(null);
                                                    }
                                                }}
                                                disabled={blockResume || resumingInteractionId === interaction.id}
                                                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-white text-xs font-semibold shadow-sm transition-all active:scale-95 ${
                                                    blockResume || resumingInteractionId === interaction.id
                                                        ? 'bg-slate-400 cursor-not-allowed'
                                                        : 'bg-blue-600 hover:bg-blue-700'
                                                }`}
                                            >
                                                {resumingInteractionId === interaction.id ? (
                                                    <>
                                                    ...
                                                    <span>Finish current first</span>
                                                ) : (
                                                    <>
