...
import { generateLabRequisitionImage } from '../utils/labRequisitionImage';
...
const MODES = [
  { id: 'print', label: 'Print the referral as a handout' },
  { id: 'email', label: 'Email the referral' },
  { id: 'fax', label: 'Fax the referral' },
];

export default function LabRequisitionActionModal({
  open,
  onClose,
  visitor,
  interactionId = '',
  ...
  const ensurePreview = async () => {
    ...
    try {
      const dataUrl = await generateLabRequisitionImage({
        ...(formPayload || {}),
        clinicianName: formPayload?.clinicianName || doctorName,
        patientFirstName: formPayload?.patientFirstName || patientName,
      });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const referralSupabasePath = await supabaseStorageService.uploadReferralForm(
        dataUrl,
        entityId,
      ...
      await outgoingLogService.create({
        ...
        referral: {
          supabasePath: referralSupabasePath,
          formType: 'lab_requisition',
        },
        notes: '',
      });
      ...
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 bg-slate-50 flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500 mt-1">What do you want to do with this referral?</p>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-slate-500">
              {isGeneratingPreview ? 'Generating preview…' : previewUrl ? 'Preview ready' : 'Preview will be generated on confirm.'}
            </div>
